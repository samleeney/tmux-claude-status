...
PATH="$FAKE_BIN:$PATH" \
HOME="$TEST_HOME" \
bash "$REPO_DIR/scripts/park-target.sh" "repo" "S"
...
if [ ! -f "$PARKED_DIR/repo.parked" ] || [ ! -f "$PARKED_DIR/repo_%1.parked" ]; then
    echo "Assertion failed: park-target should still mark the selected session as parked" >&2
    exit 1
fi

echo "park target session next inbox regression checks passed"
