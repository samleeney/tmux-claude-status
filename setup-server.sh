...
# Single script setup for SSH server agent status detection

if [ $# -ne 2 ]; then
    echo "Usage: $0 <session-name> <ssh-host>"
    echo "Example: $0 reachgpu reachgpu"
    echo "Sets up agent status tracking for the specified server"
    exit 1
fi
...
echo "Setting up agent status tracking for: $SESSION_NAME -> $SSH_HOST"
...
ssh "$SSH_HOST" "mkdir -p ~/.config/tmux/plugins/tmux-agent-status/hooks ~/.claude"

# 4. Copy hook to remote
echo "Copying hook to remote..."
scp "$SCRIPT_DIR/hooks/better-hook.sh" "$SSH_HOST:~/.config/tmux/plugins/tmux-agent-status/hooks/"
...
if [ -f "$MONITOR_SCRIPT" ]; then
    ...
    if tmux has-session -t $SESSION_NAME 2>/dev/null; then
        ssh -o ConnectTimeout=2 -o BatchMode=yes -o StrictHostKeyChecking=no -o LogLevel=QUIET \\
            $SSH_HOST \"cat ~/.cache/tmux-agent-status/${SESSION_NAME}.status\" 2>/dev/null \\
